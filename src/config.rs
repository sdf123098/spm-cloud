...
impl CloudConfig {
    pub fn from_env() -> Result<Self, CloudError> {
        let instance_id = env::var("SPM_CLOUD_INSTANCE_ID").unwrap_or_else(|_| "local-dev".to_owned());
        validate_slug(&instance_id, "instance_id")?;
        let origin = env::var("SPM_CLOUD_ORIGIN").unwrap_or_else(|_| "https://localhost".to_owned());
        let bind_addr = env::var("SPM_CLOUD_BIND")
            .unwrap_or_else(|_| "127.0.0.1:8787".to_owned())
            .parse()
            .map_err(|_| CloudError::configuration("invalid SPM_CLOUD_BIND"))?;
        let data_dir = PathBuf::from(env::var("SPM_CLOUD_DATA_DIR").unwrap_or_else(|_| "data".to_owned()));
        let database_path = env::var("SPM_CLOUD_DATABASE").map(PathBuf::from).unwrap_or_else(|_| data_dir.join("spm-cloud.db"));
        let object_dir = env::var("SPM_CLOUD_OBJECT_DIR").map(PathBuf::from).unwrap_or_else(|_| data_dir.join("objects"));
...
pub fn validate_slug(value: &str, field: &str) -> Result<(), CloudError> {
    ...
    if valid { Ok(()) } else { Err(CloudError::invalid_metadata(format!("invalid {field}"))) }
}
